...
FAKE_STAMP = """#!/usr/bin/env python3
import sys
from pathlib import Path
if "--print" in sys.argv:
    print("1.2+abc")
else:
    Path(__file__).with_name("stamped.marker").write_text("stamped")
"""

FAKE_STAMP_BROKEN = """#!/usr/bin/env python3
import sys
sys.exit(1)
"""


def make_project(root, body=FAKE_STAMP):
    """做一個「自帶版本標記腳本」的假專案目錄。"""
    tools = root / "tools"
    tools.mkdir(parents=True, exist_ok=True)
    (tools / "stamp_version.py").write_text(body, encoding="utf-8")
    return root


class TestVersionStamp:
    """待傳輸專案自帶 tools/stamp_version.py 時的版本標記行為。

    收口刻意放在 run_cli() 而不是某支 run_*.sh：發布/更新有 run_all_uploads.py、
    run_selected_transfers.py、run_radar_*.sh、手動 main.py --cli 等多條路，只在單一腳本
    裡處理的話，換一條路走就靜默失去版本資訊。
    """

    def test_no_stamp_script_leaves_version_info_untouched(self, tmp_path):
        # 其他專案（沒有 tools/stamp_version.py）完全不受影響
        assert main_module._version_stamp_script(tmp_path) is None
        assert main_module._apply_version_stamp("upload", tmp_path, "keepme") == "keepme"
        assert main_module._apply_version_stamp("upload", tmp_path, "") == ""

    def test_upload_runs_stamp_and_fills_version_info(self, tmp_path):
        root = make_project(tmp_path)
        got = main_module._apply_version_stamp("upload", root, "")
        assert got == "1.2+abc"
        # 上傳前必須真的產生標記：船上沒有 .git，算不出自己是哪個 commit
        assert (root / "tools" / "stamped.marker").is_file()

    def test_download_reads_version_without_stamping(self, tmp_path):
        root = make_project(tmp_path)
        got = main_module._apply_version_stamp("download", root, "")
        assert got == "1.2+abc"          # 記的是「下載前」的版本
        assert not (root / "tools" / "stamped.marker").exists()

    def test_explicit_version_info_is_not_overridden(self, tmp_path):
        root = make_project(tmp_path)
        assert main_module._apply_version_stamp("upload", root, "手動指定") == "手動指定"
        assert (root / "tools" / "stamped.marker").is_file()   # 標記仍然要產生

    def test_broken_stamp_script_degrades_to_unknown(self, tmp_path):
        # 版本資訊是觀測用的，腳本壞掉不該讓傳輸停擺（船上會看到 files:UNSTAMPED）
        root = make_project(tmp_path, FAKE_STAMP_BROKEN)
        assert main_module._apply_version_stamp("upload", root, "") == "unknown"

    def test_relative_local_path_is_resolved(self, tmp_path, monkeypatch):
        # 設定檔裡的 local_path 是相對路徑（radar 用 "../../radar"），需相對 cwd 解析
        make_project(tmp_path / "proj")
        monkeypatch.chdir(tmp_path)
        assert main_module._version_stamp_script("proj") is not None
