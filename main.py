...
def run_cli(args):
    ...
    # 待傳輸專案的根目錄有 VERSION.json 時：上傳前產生版本標記（含每個檔的 sha256），
    # 並把版本字串填進 log 的 version_info 欄 —— 見 version_stamp 模組的 docstring。
    # 沒有那個檔的專案完全照舊，一行都不會執行。
    # logger 尚未建立（version_info 是它的建構參數），所以那邊的訊息走 print。
    version_info = version_stamp.apply(mode, local_path, version_info, {
        "mode": "upload",
